...
Bounce( float dt )
{
	while( dt > EPSILON )
	{
		...
		float tright = (XRIGHT-(Xnow + RADIUS))/Vxnow;	// time to hit the right wall
		...
		float squareroot = sqrt((Vynow * Vynow) - (2 * Gravity) * (Ynow - YBOTTOM - RADIUS));

		float tfloor1 = (-Vynow + squareroot) / Gravity;	// time to hit the floor
		if( tfloor1 > 0.  &&  tfloor1 < tmin )
		{
			tmin = tfloor1;
			which = HIT_FLOOR1;
		}
		float tfloor2 = (-Vynow - squareroot) / Gravity;	// time to hit the floor (note there are 2 answers)
